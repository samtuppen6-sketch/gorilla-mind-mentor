...
export function prescribeBreathwork(
  ...
  const missed = /\b(missed (a |two |three |a few |some |several )?days?|fell off|fallen off|haven'?t checked in|haven'?t (done|trained)|back to square one|lost (my standard|momentum)|old me( is back)?|failed again|need to get back on track|re[- ]?enter)\b/i.test(message);
