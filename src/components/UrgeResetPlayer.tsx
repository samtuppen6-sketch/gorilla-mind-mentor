...
const BREATHING_START = 85;
const BREATHING_END = 229; // 85 + 12 * 12
const OUTRO_START = 235;
...
export function UrgeResetPlayer({ asset, started, onEnded, onClose, onCanComplete }: Props) {
  ...
  if (phase === "PREPARE") {
    ...
    // EXHALE LONG — slow, controlled contraction over 8s
  ...
  const phaseLabel =
    phase === "INHALE" ? "INHALE" : phase === "EXHALE" ? "EXHALE LONG" : "";

  const subtext =
    phase === "PREPARE"
      ? "Prepare. Move the trigger away."
      : phase === "OUTRO"
        ? "Loop broken. Standard protected."
        : phase === "INHALE"
          ? "Through the nose"
