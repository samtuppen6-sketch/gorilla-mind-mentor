...
const BREATHING_START = 64;
const BREATHING_END = 184; // 64 + 20 * 6
const OUTRO_START = 190;
const ROUND_LENGTH = 6;
const INHALE_LENGTH = 3;
const EXHALE_LENGTH = 3;
...
export function EnergisingBreathPlayer({ asset, started, onEnded, onClose, onCanComplete }: Props) {
  ...
  if (phase === "PREPARE") {
    ...
    // EXHALE WITH CONTROL — sharp 3s contraction
  ...
  const phaseLabel =
    phase === "INHALE" ? "INHALE" : phase === "EXHALE" ? "EXHALE WITH CONTROL" : "";

  const subtext =
    phase === "PREPARE"
      ? "Prepare. Switch the system on."
      : phase === "OUTRO"
        ? "System switched on. Move now."
        : phase === "INHALE"
          ? "Through the nose"
          : "With control";
