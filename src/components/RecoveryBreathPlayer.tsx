...
const BREATHING_START = 72; // 1:12
const ROUND_LENGTH = 14;
const INHALE_LENGTH = 4;
const HOLD_LENGTH = 2;
const EXHALE_LENGTH = 8;
const TOTAL_ROUNDS = 10;
const BREATHING_END = BREATHING_START + ROUND_LENGTH * TOTAL_ROUNDS; // 212 (3:32)
const OUTRO_START = 218; // 3:38
...
function computeState(t: number): {
  ...
  const round = Math.min(TOTAL_ROUNDS, roundIndex + 1);
  ...
  if (inRound < INHALE_LENGTH + HOLD_LENGTH) {
    const holdIn = inRound - INHALE_LENGTH;
    return {
      phase: "HOLD",
      countdown: Math.max(1, HOLD_LENGTH - Math.floor(holdIn)),
      round,
      phaseTime: holdIn,
    };
  }
  const exhaleIn = inRound - INHALE_LENGTH - HOLD_LENGTH;
  return {
    phase: "EXHALE",
...
export function RecoveryBreathPlayer({ asset, started, onEnded, onClose, onCanComplete }: Props) {
